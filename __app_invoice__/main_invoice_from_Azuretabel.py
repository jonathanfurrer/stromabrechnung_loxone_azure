...
try:
    credential = "Pq4gMJoZFTu9g1l43HDYrGnatgS8NDttfeajHvCPja3I5v1yUh5VcVzeZYBdfGIQ/BBGYPaVQd4+FI9abI7m1Q=="
    table_service = TableService(
        account_name="stromd41", account_key=credential)
...
def getAdressData(table, partitionKey, rowKey):
    try:
        ...
        logging.warning("failed to load adress data from " + table +
...
def getInvoice(t_strombezug, t_preise, adr_kunde, adr_an, bank_data):
    ...
    try:
        open_invoice = table_service.query_entities(
            t_strombezug, filter=f, timeout=60)
    except:
        logging.warning("failed to load adress data from " +
                        t_strombezug + " PartitionKey: " + adr_kunde.Wohnung)
    ...
    for oi in reversed(open_invoice):
        ...
        try:
            grundgebühr = table_service.query_entities(
        ...
        if "day" in oi.tarif:
            ...
            total += (float(hochtarif[0].Preis) * float(oi.value))
            subtotal = str(
                round(float(hochtarif[0].Preis) * float(oi.value), 2)) + " " + str(hochtarif[0].Währung)
        else:
            # Niedertarif / Nacht
            ...
            preis = str(niedertarif[0].Preis) + " " + \
                str(niedertarif[0].Währung)

            total += (float(niedertarif[0].Preis) * float(oi.value))
            subtotal = str(
                round(float(niedertarif[0].Preis) * float(oi.value), 2)) + " " + str(niedertarif[0].Währung)

        value = str(oi.value) + " " + str(oi.unit)
        inv.newEntity(line, MONTHS[oi.month-1], tarif, value, preis, subtotal)
        line += 1
    ...
    subtotal = str(round(
        grundgebühr[0].Preis / 12 * anzMonate, 2)) + " " + str(grundgebühr[0].Währung)

    inv.newEntity(line, periode, "Grundgebühr pro Jahr",
                  (str(anzMonate) + " Monate"), preis, subtotal)
    line += 1
    inv.newLine(line)
    line += 1
    total = round(total, 2)
...
getInvoice(TABLE_STROMBEZUG, TABLE_PREISE,
           adressData272_1, adressData272_4, bankData)
getInvoice(TABLE_STROMBEZUG, TABLE_PREISE,
           adressData272_2, adressData272_4, bankData)
getInvoice(TABLE_STROMBEZUG, TABLE_PREISE,
           adressData272_3, adressData272_4, bankData)
getInvoice(TABLE_STROMBEZUG, TABLE_PREISE,
           adressData272_4, adressData272_4, bankData)
